...
setup:
        ...
        ldr     r0, =0x40021018 @ move 0x40021018 to r0
        mov     r3, 0x11 @ loads 16 in r1 to enable clock in port C (IOPC bit)
        str     r3, [r0] @ M[RCC_APB2ENR] gets 16

        # reset pin 0 to 7 in GPIOC_CRL
        ldr     r0, =0x40011000 @ moves address of GPIOC_CRL register to r0
        ldr     r3, =0x44444444 @ this constant signals the reset state
        str     r3, [r0] @ M[GPIOC_CRL] gets 0x44444444

        # set pin 13 as digital output
        ldr     r0, =0x40011004 @ moves address of GPIOC_CRH register to r0
        ...
        ldr     r0, =0x4001100C  @ moves address of GPIOC_ODR register to r0
        mov     r1, 0x0
loop:   
...
L2:     str     r3, [r0] @ M[GPIOC_ODR] gets r1 value
        # dirty delay
        ldr     r2, =2000000 @ r2 gets 1333337
        b       L3
L4:     sub     r2, r2, #1
L3:     cmp     r2, #0
        bge     L4
